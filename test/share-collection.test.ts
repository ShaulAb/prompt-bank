...
describe('PromptService - Share Collections', () => {
  ...
  it('should fetch and parse a shared collection correctly', async () => {
    ...
    const mockPayload = Buffer.from(JSON.stringify({ prompts: originalPrompts }), 'utf8').toString(
      'base64'
    );
  ...
  it('should handle duplicate title conflicts within same category', async () => {
    ...
    expect(
      importedPrompts.find((p) => p.title === 'Test Prompt' && p.content === 'Test content')
    ).toBeDefined();
    expect(
      importedPrompts.find((p) => p.title === 'Test Prompt' && p.content === 'Different content')
    ).toBeDefined();
  ...
  it('should preserve order within categories during import', async () => {
    ...
    const allPrompts = await promptService.listPrompts();
    const architecturePrompts = allPrompts
      .filter((p) => p.category === 'Architecture')
      .sort((a, b) => (a.order || 0) - (b.order || 0));
    const generalPrompts = allPrompts
  ...
  it('should reset metadata for imported prompts', async () => {
    const originalPrompts = [createPrompt('Test Prompt', 'Test content', 'Architecture')];
